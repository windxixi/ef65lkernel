...
#define F_FW_UPDATE
#if 1
#define CE1612_UPLOADER_INFO		"CE161F00_fw1721prm1721_20111115.h"
#define CE1612_UPLOADER_BIN		"CE161F01_fw1721prm1721_20111115.h"
#define CE1612_FW_INFO				"CE161F02_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_0				"CE161F03_0_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_1				"CE161F03_1_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_2				"CE161F03_2_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_3				"CE161F03_3_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_4				"CE161F03_4_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_5				"CE161F03_5_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_6				"CE161F03_6_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_7				"CE161F03_7_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_8				"CE161F03_8_fw1721prm1721_20111115.h"
#define CE1612_FW_BIN_9				"CE161F03_9_fw1721prm1721_20111115.h"
#else
...
static struct regulator *l4b_2p8v;		//VDD_SYSIO2.8
static struct regulator *lvs1b_1p8v;		//CAM_IOVDD1.8V
static struct regulator *l15a_2p8v;		//VDD_AF2.8
...
#define C_PANTECH_CAMERA_EF65_MT9D113_STANDBY
#endif

#ifdef C_PANTECH_CAMERA_EF65_MT9D113_STANDBY
static struct regulator *lvs1b_1p8v;		//CAMV_CORE_1P8V
static struct regulator *l9a_2p8v;		//CAMV_A_2P8V
...
static int ce1612_vreg_init(void)
{
	...
	l4b_2p8v = regulator_get(NULL, "8901_l4");
	if (IS_ERR(l4b_2p8v)) {
		SKYCERR("regulator_get l4b_2p8v : 0x%x fail\n", (unsigned int)l4b_2p8v);
		return -ENODEV;
	}

	rc = regulator_set_voltage(l4b_2p8v, 2800000, 2800000);
	if (rc) {
		SKYCERR("%s: unable to set l4b_2p8v voltage to 2.8V\n", __func__);
		goto fail;
	}
	...
	if (IS_ERR(l15a_2p8v)) {
		SKYCERR("regulator_get l15a_2p8v : 0x%x fail\n", (unsigned int)l15a_2p8v);
		return -ENODEV;
	}
#if 1// (BOARD_REV >= WS10)
	rc = regulator_set_voltage(l15a_2p8v, 2800000, 2800000);
#else // for SD card
	rc = regulator_set_voltage(l15a_2p8v, 2850000, 2850000);
#endif
	if (rc) {
...
static int ce1612_l14a_off(void)
{
	...
	if (rc) {
		SKYCERR("%s: Enable regulator l15a_2p8v failed\n", __func__);
		goto fail_l14;
	}
	mdelay(1);	
	...
	if (rc){
		SKYCERR("%s: Disable regulator l15a_2p8v failed\n", __func__);
		goto fail_l14;
	}
	regulator_put(l15a_2p8v);	
...
static int ce1612_power(int on)
{
	...
	if(on) {		
...
#endif		
		//CAM_SYSIO2.8V
		rc = regulator_enable(l4b_2p8v);
		if (rc) {
			SKYCERR("%s: Enable regulator l4b_2p8v failed\n", __func__);
			goto fail;
		}
		mdelay(1);
		//CAM_IOVDD1.8
		...
		//CAM_SYSIO2.8V
		rc = regulator_disable(l4b_2p8v);
		if (rc){
			SKYCERR("%s: Disable regulator l4b_2p8v failed\n", __func__);
			goto fail;
		}
		regulator_put(l4b_2p8v);
...
//PANTECH_CAMERA_TODO
//we can control flash mode to use two gpio pins. but we don't use now
	return 0;
...
/*			data_buf[0] = 0x01;
			...
			rc = ce1612_cmd(ce1612_saddr, 0xB2, data_buf, 2);
*/
#if 0	// ï¿½ß±ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½. (auto)
			data_buf[0] = 0x03;
			data_buf[1] = 0x00;
			data_buf[2] = 0x15;
			data_buf[3] = 0x00;
...
//off CAF ICS 
	...
	{		
		...
		data_buf[1] = 0x03;
		//data_buf[2] = 0x00;
		data_buf[2] = 0x02; /* WSh. 20120823. EF65L ICS upgradeï¿½ï¿½ï¿½ï¿½ 1920*1080 ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½Ø»ï¿½ï¿½ï¿½ preview 180ï¿½ï¿½ È¸ï¿½ï¿½ï¿½Ç¾ï¿½ ï¿½ï¿½ï¿½Ì´ï¿½ ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½. */	
		rc = ce1612_cmd(ce1612_saddr, 0x54, data_buf, 3);
		if (rc < 0)
		{
			SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
	...
	else
	{
		if(ce1612_config_1080P_preview == true)
		{
			...
			SKYCDBG("%s: video size is not 1920X1080, preview=%dx%d, snapshot=%dx%d, video=%dx%d\n", __func__,
				dimension->prev_dx, dimension->prev_dy,
				dimension->pict_dx, dimension->pict_dy,
				dimension->video_dx, dimension->video_dy);
			...
			data_buf[1] = 0x01;
			//data_buf[2] = 0x00;
		    data_buf[2] = 0x02; /* WSh. 20120913. EF65L ICS µ¿¿µ»ó 1920X1080¿¡¼­ »çÁøÃÔ¿µ¸ðµå ÀüÈ¯½Ã È­¸é 180µµ rotationµÇ´Â Çö»ó ¼öÁ¤. */			
			rc = ce1612_cmd(ce1612_saddr, 0x54, data_buf, 3);
			if (rc < 0)
			{
				SKYCERR("ERR:%s FAIL!!!rc=%d return~~\n", __func__, rc);
