...
struct qt602240_data_t
{
	...
	struct work_struct work_protection_mode_enable;
...
static uint8_t qt_charger_mode_config(unsigned long mode)
{
	...
	printk("[TSP] qt_charger_mode_config [charger_mode=%d, ,previous_charger_mode=%d] \n", (int)mode,(int)previous_charger_mode);
...
void disable_autocal_locked(struct work_struct * p) {
	dbg_func_in();
	mutex_lock(&qt602240_data->lock);
	dbg("[touch] touch auto calibration disable\n");
...
void qt_Acquisition_Config_Init_locked(struct work_struct * p) {
	mutex_lock(&qt602240_data->lock);
	dbg("[touch] PROTECTION_MODE disable\n");
...
#ifdef PROTECTION_MODE
void  qt_enable_protection_mode(struct work_struct * p)
{
	int ret;
	protection_mode = true;
	mutex_lock(&qt602240_data->lock);

	acquisition_config.tchautocal= T8_TCHAUTOCAL_PROTECTION;
	acquisition_config.atchcalst = T8_ATCHCALST_PROTECTION;
	acquisition_config.atchcalsthr = T8_ATCHCALSTHR_PROTECTION;
		...
		if((fingerInfo[i].mode == TSC_EVENT_MENU) || (fingerInfo[i].mode == TSC_EVENT_HOME) || (fingerInfo[i].mode == TSC_EVENT_BACK) )  // p11223
		{
			switch (fingerInfo[i].mode) 
			{
				...
				case TSC_EVENT_BACK:
					...
					break;	
#if defined(CONFIG_MACH_MSM8960_RACERJ) || defined (CONFIG_MACH_MSM8960_STARQ)
				case TSC_EVENT_SEARCH:
					dbg("[TSP] Finger[%d] (TSC_EVENT_SEARCH = 0) - forced event\n", i);
					input_report_key(qt602240_data->input_dev, KEY_SEARCH, 0);
					break;
#endif
				default:
					break;
...
{    
	...
	if (good_calibration_cnt >= 1) {
		dbg("[TSP] good_calibration_cnt = %d \n", good_calibration_cnt);
...
int8_t check_chip_calibration(void)
{
	...
	if(get_touch_antitouch_info()) {
		...
		if ((tch_ch > 18) || (atch_ch > 15) || ((tch_ch + atch_ch) > 20)) {  
			dbg("[TSP] maybe palm, re-calibrate!! \n");
			calibrate_chip();
		} 
		// Good 
			...
			if ((finger_cnt >= 2) && (tch_ch <= 3)) {
				dbg("[TSP]finger_cnt = %d, re-calibrate!! \n", finger_cnt);	
				calibrate_chip();
			} else {
				cal_maybe_good();
				not_yet_count = 0;
			...
			dbg("[TSP] calibration was bad (finger : %d, max_antitouch_num : %d)\n", finger_cnt, finger_cnt*num_of_antitouch);
			calibrate_chip();
		} 
		// When atch > tch 
		else if((tch_ch + CAL_THR) <= atch_ch) { 
			dbg("[TSP] calibration was bad (CAL_THR : %d)\n",CAL_THR);
		...
		// No touch, only antitouch
			...
			dbg("[TSP] calibration was bad, tch_ch = %d, atch_ch = %d)\n", tch_ch, atch_ch);
			/* cal was bad - must recalibrate and check afterwards */
			calibrate_chip();
		} 
		// Else. 
			...
			not_yet_count++;
			dbg("[TSP] calibration was not decided yet, not_yet_count = %d\n", not_yet_count);
			if((tch_ch == 0) && (atch_ch == 0)) {
				not_yet_count = 0;
				//			} else if(not_yet_count >= 30) {
			} else if(not_yet_count >= 2) {		// retry count 30 -> 2
				dbg("[TSP] not_yet_count over 2, re-calibrate!! \n");
				not_yet_count =0;
				calibrate_chip();
			}
		}
...
void  get_message(struct work_struct * p)
{
	...
	if (object_type == TOUCH_MULTITOUCHSCREEN_T9)
	{
      ...
      ktime_get_ts(&ts); //TOUCH_REACTION_RATE
		...
		{
			press = 3;
			printk("[TSP][ERROR] Unknown state ! status = %d \n", quantum_msg[1]);
			goto interrupt_return;
		}
		...
		if(quantum_msg[1] == 0x10) {
			...
			dbg("[TSP] touch auto calibration start !\n");
			mod_timer(&tchautocal_timer, jiffies + msecs_to_jiffies(6000));
			}
#endif
	}
	else{
...
void report_input (void) {
		...
		if(fingerInfo[i].mode == TSC_EVENT_NONE )			// TOUCH_EVENT_PRESS (DOWN)
		{
			...
			if(fingerInfo[i].y < SCREEN_RESOLUTION_Y)	{
...
#ifdef HAS_BUTTONS// if  device has touch key such as HOME,BACK
				...
				/* ???? ?????? ???????? ?? 4mm , ?? 7mm*/
					...
					keyInfo[i].code = TSC_EVENT_SEARCH;
					keyInfo[i].status = TOUCH_EVENT_PRESS;
					keyInfo[i].update = true;
				}
...
#if defined (CONFIG_MACH_MSM8960_RACERJ) || defined (CONFIG_MACH_MSM8960_STARQ)
				else if(fingerInfo[i].mode == TSC_EVENT_SEARCH){
					fingerInfo[i].mode = TSC_EVENT_NONE;
					keyInfo[i].code = TSC_EVENT_SEARCH;
					keyInfo[i].status = TOUCH_EVENT_RELEASE;
					keyInfo[i].update = true;
...
#if defined (CONFIG_MACH_MSM8960_RACERJ) || defined (CONFIG_MACH_MSM8960_STARQ)
			else if(keyInfo[i].code == TSC_EVENT_SEARCH){
				dbg("[QT602240]S:(%d)\n", keyInfo[i].status);
				input_report_key(qt602240_data->input_dev, KEY_SEARCH, keyInfo[i].status);
			}
#endif //defined (CONFIG_MACH_MSM8960_RACERJ) || defined (CONFIG_MACH_MSM8960_STARQ)
	  input_sync(qt602240_data->input_dev);
		}
	...
	if (valid_input_count == 0 && cal_correction_limit > 0) {
		dbg("[TOUCH] PROTECTION_MODE start\n");
		//queue_work(qt602240_wq, &qt602240_data->work_protection_mode_enable);
		acquisition_config.atchcalst = 0;
		acquisition_config.atchcalsthr = 0; // Never suspend
		acquisition_config.atchfrccalthr = 50;
		acquisition_config.atchfrccalratio = 20;
...
#ifdef PROTECTION_MODE 
	cancel_work_sync(&qt602240_data->work_protection_mode_enable);
...
#ifdef PROTECTION_MODE
	INIT_WORK(&qt602240_data->work_protection_mode_enable, qt_enable_protection_mode);
