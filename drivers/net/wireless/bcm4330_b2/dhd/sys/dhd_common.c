...
// FEATURE_SKY_DS_WIFI_BC_DIRECT_INTERFACE_RENAME
...
// FEATURE_SKY_DS_WIFI_BC_DIRECT_INTERFACE_RENAME
							...
							if( !ap_fw_loaded )
							{
								...
								if (0 != ifevent->ifidx) {
									...
									snprintf(p2piface_name, IFNAMSIZ,
										"%s%d", P2P_IFNAME, ifevent->ifidx);
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	uint bcn_timeout = 4;
	int arpoe = 1;
#ifdef CONFIG_BCM4330_HT_WEP_ENABLE
	uint ht_wep = 0;
#endif
...
#ifdef CUSTOMER_HW_PT
	int scan_unassoc_time = 80; //60->80
#else
...
#ifdef CONFIG_BCM4330_ROAM_ENABLE
    {
		...
		roam_val.band = WLC_BAND_ALL;
		roam_val.val = roam_delta;
		if (dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_DELTA, \
				(char *)&roam_val, sizeof(roam_val), TRUE, 0) < 0)
				DHD_ERROR(("%s: roam delta setting failed\n", __FUNCTION__));

		roam_val.val = roam_trigger;
...
#ifdef CUSTOMER_HW_PT
        /* Set keep_alive */
        if(dhd->mkeep_alive.period_msec > 0) {
                wl_mkeep_alive_pkt_t pkt;

                pkt.period_msec = dhd->mkeep_alive.period_msec;
				pkt.keep_alive_id = dhd->mkeep_alive.keep_alive_id;
				pkt.version = dhd->mkeep_alive.version;
				pkt.length = dhd->mkeep_alive.length;
                pkt.len_bytes = 0;

                buf_len = bcm_mkiovar("mkeep_alive",
                                (char*)&pkt, sizeof(wl_mkeep_alive_pkt_t),
                                buf, sizeof(buf));

                if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, buf, buf_len, TRUE, 0) < 0)
                {
                        DHD_ERROR(("%s:can't set mkeep alive\n", __FUNCTION__));
                }
        }
#endif
...
#ifdef CONFIG_BCM4330_HT_WEP_ENABLE
        /* set allow_ht_wep */
        bcm_mkiovar("ht_wsec_restrict", (char *)&ht_wep, 4, iovbuf, sizeof(iovbuf));
        dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#endif
