...
extern void MHL_Set_Cable_State(bool connect, bool enable);
...
static int msm_otg_suspend(struct msm_otg *dev)
{
	...
	{
#if	0
	MHL_En_Control(0); // switch-USB	
	mhl_power_ctrl(0);
	MHL_On(MHL_DEVICE_OFF);  
	
	MHL_Cable_On(MHL_CABLE_DISCONNCET);
	MHL_Set_Cable_State(MHL_CABLE_DISCONNCET);
#else	
		MHL_Set_Cable_State(MHL_CABLE_DISCONNCET, true);
#endif
...
{
	MHL_Set_Cable_State(MHL_CABLE_CONNCET, true);
