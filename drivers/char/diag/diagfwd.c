...
enum
{
  ...
  FACTORY_CS_AUTO_LAYER1_I       = 2, // 0x2-진동시간, 벨 재생 시간, 키 눌린 횟수
  FACTORY_CS_AUTO_LAYER2_I       = 3, // 0x3-Slide(Folder) 개폐 횟수, 젠더 체결 횟수
  FACTORY_CS_AUTO_LAYER3_I       = 4, // 0x4-카메라 구동 시간, LCD Backlight 점등시간
  FACTORY_CS_AUTO_MEMORY_I       = 5, // 0x5-메모리 사용 용량
  FACTORY_CS_AUTO_CALL1_I        = 6, // 0x6-총 통화 시간/건수, 영상 통화 시간
  FACTORY_CS_AUTO_CALL2_I        = 7, // 0x7-SMS 수/발신 건수, WAP 접속 건수
  FACTORY_CS_AUTO_CALL3_I        = 8, // 0x8-무선인터넷 시간, NO SVC/Call Drop 발생 횟수
  FACTORY_CS_AUTO_RESET_I        = 9, // 0x9-s/w reset, 전원 ON/OFF 횟수, h/w reset
  FACTORY_CS_AUTO_ERRLOG_I       = 10, // 0xA-에러 로그
  FACTORY_CS_AUTO_CALL4_I        = 11, // 0xB-Call drop 및 발신 실패
  FACTORY_CS_AUTO_CALL5_I        = 12, // 0xC-발신 및 착신 성공
  FACTORY_MSECTOR_WRITE_I        = 20, // 0x14-MSector Write Flag
  FACTORY_MSECTOR_READ_I         = 21, // 0x15-MSector Read Flag
  FACTORY_CHECK_CAL_FLAG_I       = 30, // 0x1E-Cal 완료여부 확인
  FACTORY_FIRMWARE_VER_I         = 39, // 0x27-FIRMWARE 버전 확인
  FACTORY_M3A_NAND_VER_I         = 40, // 0x28-2nd NAND/TCC Version 버전 확인
  FACTORY_FINAL_I                = 48, // 0x30-출하설정관련 초기화
  FACTORY_FINAL_CHECK_I          = 49, // 0x31-출하설정관련 확인
  FACTORY_INIT_ALL_I             = 55, // 0x37-전체 초기화.
  FACTORY_INIT_RESET_I           = 56, // 0x38-초기화 완료 후 H/W 리셋
  FACTORY_INIT_SYS_CHECK_I       = 58, // 0x3A-시스템초기화 확인
  ...
  FACTORY_BT_TEST_SET_I          = 92, // 0x5C-Bluetooth 모드 설정
  FACTORY_BT_TEST_RELEASE_I      = 93, // 0x5D-Bluetooth 모드 해제
  FACTORY_BT_TEST_CHECK_I        = 94, // 0x5E-Bluetooth 모드 확인
  FACTORY_WIFI_MODESET_I         = 96, // 0x60-RX/TX DUT Mode설정
  FACTORY_WIFI_TXCTRL_I          = 97, // 0x61-TX Mode 설정 후 TX Power 제어(ON/OFF)
  FACTORY_WIFI_RXRPT_I           = 98, // 0x62-RX Mode 설정 후 BER 측정
  FACTORY_WIFI_RELEASE_I         = 99, // 0x63-Test 완료 후 Test Mode 해제
  FACTORY_BOOT_CHECK_I           = 100, // 0x64-부팅이 완료되어서 TEST가능 모드인지 확인
  FACTORY_KEY_PRESS_I            = 110, // 0x6E-Command를 이용하여 키를 동작시킴
  FACTORY_LCD_I                  = 115, // 0x73-LCD를 On/Off
  FACTORY_SPEAKER_I              = 120, // 0x78-Speaker를 이용해 음원을 출력
  FACTORY_IRDA_I                 = 125, // 0x7D-IRDA Port로 데이터 송수신 Test
  FACTORY_BATTERY_BAR_I          = 135, // 0x87-Battery bar 개수를 알아냄
  FACTORY_BATTERY_ADC_I          = 136, // 0x88-Battery ADC Value
  FACTORY_CUTOFF_SWITCH_I        = 137, // 0x89-Battery Cutoff ADC 체크 비활성화
  FACTORY_SLEEP_MODE_I           = 140, // 0x8C-Sleep Mode Enable/Disable
  FACTORY_TEMP_I                 = 145, // 0x91-온도 값 Read
  FACTORY_GPS_APP_MAP_CHECK_I    = 154, // 0x9A-GPS App, Map 상태 확인
  FACTORY_CONTENTS_CHECK_I       = 155, // 0x9B-Contents 이상 유무 확인
  FACTORY_AGPS_TEST_SET_I        = 156, // 0x9C A-GPS C/No Test 모드 설정
  FACTORY_AGPS_TEST_CHECK_I      = 157, // 0x9D A-GPS C/No Test 모드 확인
  FACTORY_AGPS_MEASURE_I         = 158, // 0x9E A-GPS C/No측정값 확인
  FACTORY_AGPS_TEST_RELEASE_I    = 159, // 0x9F C/No Test 모드 해제 
  FACTORY_CHECKSUM_READ_I        = 160, // 0xA0-SECTION 별 CHECKSUM 계산
  FACTORY_USIM_MODE_SET_I        = 176, // 0xB0-USIM Mode설정
  FACTORY_USIM_MODE_CHECK_I      = 177, // 0xB1-USIM Mode 확인
  FACTORY_USIM_CARD_CHECK_I      = 178, // 0xB2-USIM Card 확인
  FACTORY_USIM_PERSO_CONTROL_I   = 179, // 0xB3_ME Personalization을 제어
  FACTORY_SDCARD_CHECK_I         = 180, // 0xB4-SD Card 삽입유무 및 용량 확인
  FACTORY_DUAL_USIM_CARD_CHECK_I = 181, // 0xB5-Dual USIM Card 확인
  FACTORY_BAUDRATE_CHANGE_I      = 200, // 0xC8-Baudrate change
  FACTORY_VOLUME_MAX_I           = 201, // 0xC9-Voice Call 음량 최대로 설정
  FACTORY_SERIAL_NUMBER_I        = 202, // 0xCA-일본향 모델 제조번호 Read/Write Command
  FACTORY_MODEL_NAME_I           = 203, // 0xCB-모델명 Read Command
  FACTORY_PMIC_INIT_I            = 204, // 0xCC-PMIC 초기화 명령.
  FACTORY_PIN_CODE_READ_I        = 210, // 0xD2-일본향 PIN Code Read 명령
  FACTORY_WCDMA_MIN_READ_I       = 211, // 0xD3-내수향 WCDMA모델 전화번호를 읽어오는 명령
  FACTORY_MOVIENAND_FORMAT_I     = 215, // 0xD7 MovieNAND Memory Format Command
  FACTORY_SHORTCUT_DIAL_I        = 216, // 0xD8 Auto Call 공정에서 단말기 -> 계측기로 강제로 단축 Call(911또는…) 걸기 위한 Command
  FACTORY_IMEI_RD_I              = 220, // Dual USIM의 경우First IMEI Read Command
  FACTORY_IMEI_WR_I              = 221, // Dual USIM의 경우First IMEI Write Command
  FACTORY_IMEI_2_RD_I            = 222, // Dual USIM의 경우Second IMEI Read Command
  FACTORY_IMEI_2_WR_I            = 223, // Dual USIM의 경우Second IMEI Write Command
  FACTORY_WIFI_MAC_RD_I          = 224, // WiFi Mac Read Command
  FACTORY_WIFI_MAC_WR_I          = 225, // WiFi Mac Write Command
  FACTORY_HDET_AUTO_CAL_I        = 226, // MSM, MDM HDET Auto Cal을 폰 내부적으로 수행
  FACTORY_RF_PATH_INIT_I         = 227, // RF Switch 초기화. 이거 안하면 MDM Rx Div 조정 안됨
  FACTORY_ANDROID_SHUTDOWN_I     = 230, // File System 동기화를 위한 Command
  FACTORY_PATTERN_UNLOCK_I       = 231, // PATTERN 잠금 설정 해제
  FACTORY_WIFI_AP_INFO_I         = 232, // Scan된 AP의 SSID 및 RSSI Level 확인
  FACTORY_WIFI_CONNECT_I         = 233, // SSID를 이용하여 해당 AP에 연결하기
  FACTORY_RX_AGC_VERIFY_I        = 234, // RX AGC 확인 Command
  FACTORY_SENSOR_CHECK_I         = 235, // 적용된 Sensor의 인식 여부를 확인하여 결과값 Return
  FACTORY_DO_NV_BACKUP_I         = 236, // NV를 백업 영역에 복사
  FACTORY_SET_CAL_FLAG_I         = 237, // 조정 완료 Flag 설정 ? Set / Reset
  FACTORY_GET_OPER_MODE_I        = 238,
  FACTORY_MONITORING_ONOFF_I     = 240, // 0xF0-일본향 Usb및 serial diag통신시 폰내부적으로 모니터링 데이터가 발생하는걸 방지하는 명령
  FACTORY_FUNCTION_FLAG_I        = 242, // FUNCTION TEST후 결과가 PASS/FAIL인 항목에 대해 TEST 완료후 그 결과를 FLAG로 저장
  FACTORY_GS_COMPENSATION_I      = 243, // GlideSensor Compensation Command
  FACTORY_SYSTEM_PARAMETER_I     = 244, // System Parameter 확인 Command
  FACTORY_MICP_CONTROL_I         = 245, // 1st, 2nd MIC Path Control(Enable/Disable)
  FACTORY_ICC_ID_I               = 246, // 월드폰 Simcard Serial Number ID Read Command
  FACTORY_PREF_MODE_I            = 247, // prefer mode 설정 Command
  ...
  FACTORY_TEMPMIN_RD_I           = 252, // 일본향 모델 TempMIN Read Command
  FACTORY_TEMPMIN_WR_I           = 253, // 일본향 모델 TempMIN Write Command
  FACTORY_DMP_CHANGE_I           = 254, // DM Port 변경 Command
